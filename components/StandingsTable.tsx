    ...
    return (
      <div className="flex items-center justify-center py-24">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm" style={{ color: '#fff' }}>Loading standings from ESPN…</span>
        </div>
      </div>
    )
  }
    ...
    <div>
        ...
        <div className="flex gap-1">
          <button
            ...
            className={`px-3 py-1.5 rounded text-sm font-medium transition-colors ${
              activeTab === 'standings' ? 'bg-orange-500 text-white' : 'hover:text-white'
            }`}
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded text-sm font-medium transition-colors flex items-center gap-1.5 ${
              activeTab === 'shutout' ? 'bg-orange-500 text-white' : 'hover:text-white'
            }`}
      ...
      {activeTab === 'standings' && (
        <div>
          <table className="w-full text-sm">
            <thead>
              <tr
                className="text-xs uppercase tracking-wider border-b border-zinc-700"
                style={{ color: '#93b4e0' }}
              >
                ...
                <>
                  ...
                  >
                    <td className="px-3 py-3">
                      ...
                      }`} style={{ color: r.rank === 1 ? undefined : '#cbd5e1' }}>
                        {r.rank}
                      </span>
                    </td>
                    <td className="px-2 py-2.5">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs hidden sm:inline" style={{ color: '#fff' }}>
                          {r.members}
                        </span>
                      </div>
                      <div className="text-xs mt-0.5 sm:hidden pl-0.5" style={{ color: '#fff' }}>
                        {r.members}
                      </div>
                    </td>
                    <td className="text-center px-2 py-3 font-mono text-xs hidden sm:table-cell"
                      style={{ color: '#cbd5e1' }}>
                      {r.gamesPlayed > 0 ? r.gamesPlayed : '—'}
                    </td>
                    <td
                      className="text-center px-2 py-3 font-mono text-xs font-bold hidden sm:table-cell"
                      style={{ color: r.goalsFor === 0 && r.gamesPlayed > 0 ? '#fbbf24' : '#cbd5e1' }}
                    >
                    ...
                    <td className="text-center px-2 py-3">
                      <span className={`text-xs px-2 py-1 rounded font-medium ${STAGE_BADGE[r.stage] ?? ''}`}>
                        {STAGE_DISPLAY[r.stage] ?? r.stage}
                      </span>
                    </td>
                    <td className="text-center px-2 py-3">
                      <span
                        className={`text-xs inline-block transition-transform duration-200 ${
                    ...
                    <tr key={`${r.team}-drilldown`}>
                      <td colSpan={7} className="p-0 bg-zinc-900">
                        {r.groupStandings ? (
                          ...
                          <div className="px-6 py-3 text-xs italic" style={{ color: '#cbd5e1' }}>
          ...
          <div className="px-4 py-3 text-xs border-t border-zinc-700" style={{ color: '#93b4e0' }}>
      ...
      {activeTab === 'shutout' && (
        <div className="p-4 space-y-4">
              ...
              {shutoutWinners.map((r) => (
                <div key={r.team} className="flex items-center justify-between py-2.5 border-b border-zinc-700">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs" style={{ color: '#fff' }}>{r.members}</span>
                  </div>
            ...
            <div>
              ...
              {shutoutCandidates.map((r) => (
                <div key={r.team} className="flex items-center justify-between py-2.5 border-b border-zinc-700/50">
                  ...
                  <span className="text-xs text-amber-400">{r.gamesPlayed}/3 played</span>
                </div>
              ))}
            </div>
          )}
          ...
          <p className="text-xs pt-2" style={{ color: '#93b4e0' }}>
